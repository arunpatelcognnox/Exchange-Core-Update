 ...
 */
package com.arun.exchange.core2.core.processors.journaling;
...
import com.arun.exchange.core2.core.common.config.InitialStateConfiguration;
import com.arun.exchange.core2.core.common.config.PerformanceConfiguration;
...
//        log.debug("Writing {}", cmd);
        ...
        if (cmdType == OrderCommandType.MOVE_ORDER) {
            ...
            buffer.putDouble(cmd.price); // 8 bytes - can be compressed as delta
            ...
            buffer.putInt(cmd.symbol); // 4 bytes can be compressed as dictionary
            buffer.putLong(cmd.orderId); // 8 bytes - can be compressed as delta
            buffer.putDouble(cmd.size); // 8 bytes - can be compressed as low value
            ...
            buffer.putLong(cmd.orderId); // 8 bytes - can be compressed as delta
            buffer.putDouble(cmd.price); // 8 bytes - can be compressed as delta
            buffer.putDouble(cmd.reserveBidPrice); // 8 bytes - can be compressed (diff to price or 0)
            buffer.putDouble(cmd.size); // 8 bytes - can be compressed
            ...
            buffer.putDouble(cmd.price); // 8 bytes - can be compressed as low value (amount)
...
//                    dSeq + baseSeq, (byte) cmd.symbol, cmd.orderId, cmd.price, cmd.reserveBidPrice, cmd.size, cmd.uid));
            ...
            buffer.putDouble(cmd.price); // 8 bytes word1
            buffer.putDouble(cmd.reserveBidPrice); // 8 bytes word2
            buffer.putDouble(cmd.size); // 8 bytes word3
