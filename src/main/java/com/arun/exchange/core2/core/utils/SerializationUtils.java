...
//        log.debug("COMPRESS originalSize={}", originalSize);
    ...
    public static double[] bytesToDoubleArrayLz4(final LZ4Compressor lz4Compressor, final NativeBytes<Void> bytes, final int padding) {
        int originalSize = (int) bytes.readRemaining();
//        log.debug("COMPRESS originalSize={}", originalSize);
    ...
    public static double[] toDoubleArray(final byte[] bytes, final int offset, final int length, final int padding) {

        final int longLength = requiredLongArraySize(length, padding);
        double[] longArray = new double[longLength];
        //log.debug("byte[{}]={}", bytes.length, bytes);
        final ByteBuffer allocate = ByteBuffer.allocate(longLength * 8 * 2);
        final DoubleBuffer longBuffer = allocate.asDoubleBuffer();
        allocate.put(bytes, offset, length);
        longBuffer.get(longArray);
        return longArray;
    }
    ...
    public static Wire longsLz4ToWire(double[] dataArray, int longsTransfered) {
...
//        log.debug("long dataArray.len={} longsTransfered={}", dataArray.length, longsTransfered);
        ...
        byteBuffer.asDoubleBuffer().put(dataArray, 0, longsTransfered);
        ...
        System.out.println("lz4FastDecompressor");
    ...
    public static void marshallDoubleArray(final double[] longs, final BytesOut bytes) {
        bytes.writeInt(longs.length);
        for (double word : longs) {
            bytes.writeDouble(word);
        }
    }
    ...
    public static double[] readDoubleArray(final BytesIn bytes) {
        final int length = bytes.readInt();
        final double[] array = new double[length];
        // TODO read byte[], then convert into long[]
        for (int i = 0; i < length; i++) {
            array[i] = bytes.readDouble();
        }
        return array;
    }
    ...
    public static void marshallIntDoubleHashMap(final MutableIntDoubleMap hashMap, final BytesOut bytes) {
    ...
    public static IntDoubleHashMap readIntDoubleHashMap(final BytesIn bytes) {
        int length = bytes.readInt();
        final IntDoubleHashMap hashMap = new IntDoubleHashMap(length);
        // TODO shuffle (? performance can be reduced if populating linearly)
        for (int i = 0; i < length; i++) {
            int k = bytes.readInt();
            long v = bytes.readLong();
    ...
    public static <T extends WriteBytesMarshallable> void marshallDoubleHashMap(final DoubleObjectHashMap<T> hashMap, final BytesOut bytes) {
    ...
    public static <T> DoubleObjectHashMap<T> readDoubleHashMap(final BytesIn bytes, final Function<BytesIn, T> creator) {
        int length = bytes.readInt();
        final DoubleObjectHashMap<T> hashMap = new DoubleObjectHashMap<>(length);
        for (int i = 0; i < length; i++) {
            hashMap.put(bytes.readDouble(), creator.apply(bytes));
        }
        return hashMap;
    }
    ...
    public static <T extends WriteBytesMarshallable> void marshallDoubleMap(final Map<Double, T> map, final BytesOut bytes) {
        bytes.writeInt(map.size());
    ...
    public static <T, M extends Map<Double, T>> M readDoubleMap(final BytesIn bytes, final Supplier<M> mapSupplier, final Function<BytesIn, T> creator) {
        int length = bytes.readInt();
        final M map = mapSupplier.get();
        for (int i = 0; i < length; i++) {
            map.put(bytes.readDouble(), creator.apply(bytes));
        }
        return map;
    }
    ...
    public static IntDoubleHashMap mergeSum(final IntDoubleHashMap... maps) {
    	IntDoubleHashMap res = null;
        for (IntDoubleHashMap map : maps) {
            if (map != null) {
                if (res == null) {
                    res = new IntDoubleHashMap(map);
                } else {
                    map.forEachKeyValue(res::addToValue);
                }
            }
        }
        return res != null ? res : new IntDoubleHashMap();
    }
