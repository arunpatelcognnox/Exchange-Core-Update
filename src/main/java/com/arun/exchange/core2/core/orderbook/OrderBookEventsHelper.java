...
import static com.arun.exchange.core2.core.ExchangeCore.EVENTS_POOLING;
...
//        log.debug("** sendTradeEvent: price:{} v:{}", price, v);
        ...
        RepostoryService.saveTradeEventInRepository(event);
        
        return event;

    }

    public MatcherTradeEvent sendReduceEvent(final IOrder order, final double reduceSize, final boolean completed) {
//        log.debug("Cancel ");
        final MatcherTradeEvent event = newMatcherEvent();
...
//        event.size = order.getSize() - order.getFilled();
    ...
    public void attachRejectEvent(final OrderCommand cmd, final double rejectedSize) {
...
//            log.debug("BIN EVENT: {}", event);
    ...
    public static NavigableMap<Integer, Wire> deserializeEvents(final OrderCommand cmd) {
        ...
        sections.forEach((section, events) -> {
            final double[] dataArray = events.stream()
