...
import com.arun.exchange.core2.core.common.L2MarketData;
import com.arun.exchange.core2.core.common.MatcherEventType;
...
import com.arun.exchange.core2.core.common.SymbolPositionRecord;
import com.arun.exchange.core2.core.common.SymbolType;
import com.arun.exchange.core2.core.common.UserProfile;
import com.arun.exchange.core2.core.common.api.binary.BatchAddAccountsCommand;
...
import com.arun.exchange.core2.core.common.api.reports.ReportResult;
...
import com.arun.exchange.core2.core.common.config.LoggingConfiguration;
import com.arun.exchange.core2.core.common.config.OrdersProcessingConfiguration;
...
public final class RiskEngine implements WriteBytesMarshallable {
    ...
    private final IntDoubleHashMap fees;
    private final IntDoubleHashMap adjustments;
    private final IntDoubleHashMap suspends;
        ...
        if (exchangeConfiguration.getInitStateCfg().fromSnapshot() && Files.exists(SnapshotPath)) {
            ...
            final State state = serializationProcessor.loadData(
                    ...
                    bytesIn -> {
                        ...
                        final IntDoubleHashMap fees = SerializationUtils.readIntDoubleHashMap(bytesIn);
                        final IntDoubleHashMap adjustments = SerializationUtils.readIntDoubleHashMap(bytesIn);
                        final IntDoubleHashMap suspends = SerializationUtils.readIntDoubleHashMap(bytesIn);
            ...
            this.fees = new IntDoubleHashMap();
            this.adjustments = new IntDoubleHashMap();
            this.suspends = new IntDoubleHashMap();
        }
    ...
    public static class LastPriceCacheRecord implements BytesMarshallable, StateHash {
        public double askPrice = Double.MAX_VALUE;
        public double bidPrice = 0L;
        ...
        public LastPriceCacheRecord(BytesIn bytes) {
            this.askPrice = bytes.readDouble();
            this.bidPrice = bytes.readDouble();
        }

        @Override
        public void writeMarshallable(BytesOut bytes) {
            bytes.writeDouble(askPrice);
            bytes.writeDouble(bidPrice);
        }

        public LastPriceCacheRecord averagingRecord() {
            LastPriceCacheRecord average = new LastPriceCacheRecord();
            System.out.println("if any error");
            average.askPrice = (long)(this.askPrice + this.bidPrice) >> 1;
    ...
    public boolean preProcessCommand(final long seq, final OrderCommand cmd) {
        switch (cmd.command) {
            ...
            case BINARY_DATA_QUERY:
            	System.out.println("match engine +> "+ cmd);
    ...
    private CommandResultCode adjustBalance(long uid, int currency, double amountDiff, long fundingTransactionId, BalanceAdjustmentType adjustmentType) {
        System.out.println("balnce adrjest");
    ...
    private CommandResultCode placeExchangeOrder(final OrderCommand cmd,
        ...
        final double size = cmd.size;
        final double orderHoldAmount;
        ...
        double newBalance = userProfile.accounts.addToValue(currency, -orderHoldAmount);
...
//            log.warn("orderAmount={} > userProfile.accounts.get({})={}", orderAmount, currency, userProfile.accounts.get(currency));
    ...
    private boolean canPlaceMarginOrder(final OrderCommand cmd,
        ...
        final double newRequiredMarginForSymbol = position.calculateRequiredMarginForOrder(spec, cmd.action, cmd.size);
...
//                newRequiredMarginForSymbol, position.currency, userProfile.accounts.get(position.currency), freeMargin);
    ...
    private void handleMatcherEventMargin(final MatcherTradeEvent ev,
        ...
        if (takerUp != null) {
            if (ev.eventType == MatcherEventType.TRADE) {
                ...
                final double sizeOpen = takerSpr.updatePositionForMarginTrade(takerAction, ev.size, ev.price);
                final double fee = spec.takerFee * sizeOpen;
        ...
        if (ev.eventType == MatcherEventType.TRADE && uidForThisHandler(ev.matchedOrderUid)) {
            ...
            double sizeOpen = makerSpr.updatePositionForMarginTrade(takerAction.opposite(), ev.size, ev.price);
            final double fee = spec.makerFee * sizeOpen;
    ...
    private void handleMatcherEventsExchangeSell(MatcherTradeEvent ev,
        ...
        while (ev != null) {
            ...
            if (uidForThisHandler(ev.matchedOrderUid)) {
                final double size = ev.size;
                final UserProfile maker = userProfileService.getUserProfileOrAddSuspended(ev.matchedOrderUid);

                // buying, use bidderHoldPrice to calculate released amount based on price difference
                final double priceDiff = ev.bidderHoldPrice - ev.price;
                final double amountDiffToReleaseInQuoteCurrency = CoreArithmeticUtils.calculateAmountBidReleaseCorrMaker(size, priceDiff, spec);
                maker.accounts.addToValue(quoteCurrency, amountDiffToReleaseInQuoteCurrency);

                final double gainedAmountInBaseCurrency = CoreArithmeticUtils.calculateAmountAsk(size, spec);
    ...
    private void handleMatcherEventsExchangeBuy(MatcherTradeEvent ev,
    	...
    	double takerSizeForThisHandler = 0L;
        double makerSizeForThisHandler = 0L;

        double takerSizePriceSum = 0L;
        double takerSizePriceHeldSum = 0L;
        ...
        while (ev != null) {
            ...
            if (uidForThisHandler(ev.matchedOrderUid)) {
                final double size = ev.size;
                final UserProfile maker = userProfileService.getUserProfileOrAddSuspended(ev.matchedOrderUid);
                final double gainedAmountInQuoteCurrency = CoreArithmeticUtils.calculateAmountBid(size, ev.price, spec);
    ...
    public void writeMarshallable(BytesOut bytes) {
        ...
        SerializationUtils.marshallIntDoubleHashMap(fees, bytes);
        SerializationUtils.marshallIntDoubleHashMap(adjustments, bytes);
        SerializationUtils.marshallIntDoubleHashMap(suspends, bytes);
    }
